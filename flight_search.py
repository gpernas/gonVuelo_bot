...
async def search_flights(origin, destination, date_out, date_in=None, passengers=1):
    """
    Busca vuelos con Amadeus. Busca siempre con 1 adulto y filtra
    por numberOfBookableSeats >= passengers para garantizar que
    todo el grupo pueda ir en el mismo vuelo.
    """
    def _search():
    ...
    try:
        ...
        # Filtrar vuelos con plazas suficientes
        available = [
            f for f in flights
        ...
        return available[:5]  # Top 5 opciones
...
def build_deeplink(airline_code, origin, destination, date_out, date_in, passengers):
    """Genera deep links directos a cada aerolínea con los parámetros precargados."""
    links = {
        'FR': (
    ...
    # Fallback genérico para aerolíneas sin deep link específico
    default = (
        f"https://www.google.com/flights?hl=es#flt={origin}.{destination}.{date_out}"
        f"{'*' + destination + '.' + origin + '.' + date_in if date_in else ''}"
        f";c:EUR;e:1;px:{passengers};sd:1;t:f"
    )
    return links.get(airline_code, default)


def format_duration(minutes_str):
    """Convierte 'PT2H30M' en '2h 30min'"""
    import re
    match = re.match(r'PT(?:(\d+)H)?(?:(\d+)M)?', minutes_str)
    if not match:
        return minutes_str
...
def format_flight_message(flights, origin, destination, date_out, date_in, passengers):
    """
    Devuelve lista de (mensaje, teclado) — uno por vuelo encontrado.
    """
    results = []
    ...
    for i, flight in enumerate(flights, 1):
        ...
        keyboard = [[InlineKeyboardButton(
            f"🔗 Reservar en {airline_name}.com →",
            url=deeplink
        )]]

        results.append((msg, keyboard))
